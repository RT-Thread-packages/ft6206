...
static rt_thread_t  ft6206_thread = RT_NULL;
static rt_sem_t     ft6206_sem = RT_NULL;
static rt_device_t  dev = RT_NULL;
static struct       rt_touch_data *read_data;
static struct       rt_touch_info info;

static void ft6206_entry(void *parameter)
{
    ...
    while (1)
    {
        rt_sem_take(ft6206_sem, RT_WAITING_FOREVER);
...
{
    rt_sem_release(ft6206_sem);
    rt_device_control(dev, RT_TOUCH_CTRL_DISABLE_INT, RT_NULL);
    return 0;
}

/* Test function */
int ft6206_sample(const char *name, rt_uint16_t x, rt_uint16_t y)
{
    void *id;
    ...
    ft6206_sem = rt_sem_create("dsem", 0, RT_IPC_FLAG_FIFO);

    if (ft6206_sem == RT_NULL)
    {
    ...
    if (ft6206_thread != RT_NULL)
        rt_thread_startup(ft6206_thread);
